...
function GroupInfoModal({
  ...
  // ✅ FIX: Call all Hooks at the top level, before any returns.
  ...
  // ✅ FIX: Use useEffect to safely update state when the 'group' prop is available or changes.
  useEffect(() => {
    if (group) {
  ...
  // Now that hooks are called, we can safely return early if there's no group.
  if (!group) {
    return null;
  }

  const groupMessages =
    messages && group && group.id ? messages[group.id] || [] : [];

  const mediaMessages = groupMessages.filter((m) => m.type === "image");

  const fileTypes = ["pdf", "word", "excel", "ppt", "other"];
  const fileMessages = groupMessages.filter(
    (m) => m.type === "file" && fileTypes.includes(m.filetype)
  );

  const urlRegex = /(https?:\/\/[^\s]+)/g;
  const linkMessages = groupMessages.filter(
    (m) => typeof m.text === "string" && urlRegex.test(m.text)
  );
  ...
  return (
    <>
      <div className="fixed inset-0    bg-opacity-100 flex items-center justify-center z-[60]">
        <div className="bg-white  rounded-lg shadow-2xl w-[26rem] h-[90vh] max-h-[80vh] flex overflow-hidden relative">
          <button
            onClick={onClose}
          ...
          <div className="w-40 border-r bg-gray-50 p-4 space-y-2 overflow-y-auto">
            {["overview", "members", "media", "files", "links"].map((tab) => (
              <button
                key={tab}
                className={`capitalize w-full text-left px-3 py-1 rounded ${
                  activeTab === tab
                  ...
                  {isGroup && isCurrentUserAdmin && (
                    <div className="mb-3">
                      <button
                        ...
                        className="w-full bg-blue-600 cursor-pointer hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded"
                      >
                        ⚙️ Manage Members
                      </button>
                    </div>
      ...
      {showPreview && (
        <div
          className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[70]"
