...
const NavbarSkeleton = ({ theme }) => (
  <header className={`${theme === 'dark' ? 'bg-black' : 'bg-white'} px-6 py-3 shadow-md flex items-center justify-between w-full fixed top-0 left-0 z-50 h-16 animate-pulse`}>
...
const Navbar = ({ setSidebarOpen }) => {
  ...
  const { unreadCount, userData, userprofiledata, theme, setTheme } = useContext(Context);  
  ...
  return (
    <header className={`${theme === 'dark' ? 'bg-black' : 'bg-white'} px-6 py-3 shadow-md flex items-center justify-between w-full fixed top-0 left-0 z-50 h-16`}>
      <div className="flex items-center space-x-4">
        <button
          className={`md:hidden text-2xl ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`}
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            {userprofiledata?.employeeImage ? (
              ...
              <span className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-600'}`}>{initials}</span>
            )}
          </div>
          ...
          {/* Dropdown Menu */}
          {dropdownOpen && (
            <div className={`absolute right-0 mt-2 w-56 rounded-md shadow-lg z-[200] ${
              ...
              <div className="py-1">
                ...
                {/* Settings Section */}
                <div className={`border-t ${theme === 'dark' ? 'border-gray-600' : 'border-gray-200'} mt-1 pt-1`}>
                  <div className={`flex items-center justify-between px-4 py-2 text-sm ${
                    ...
                    <div 
                      ...
                      className={`relative inline-flex items-center h-5 w-9 cursor-pointer rounded-full transition-colors duration-200 focus:outline-none ${
